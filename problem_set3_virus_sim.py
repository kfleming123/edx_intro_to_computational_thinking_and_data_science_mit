...
class SimpleVirus(object):
    ...
    def doesClear(self):
        ...
        return self.getClearProb() >= chance
    ...
    def reproduce(self, popDensity):
        ...
        if self.getMaxBirthProb() * (1 - popDensity) >= chance2:
...
class Patient(object):
    ...
    def update(self):
        ...
        pop_density = self.getTotalPop() / self.getMaxPop()
        for v in self.getViruses():
            try:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    patient1 = Patient(viruses, maxPop)
    num_viruses = [0] * 300
    for i in range(numTrials):
        for j in range(300):
            patient1.update()
            num_viruses[j] += patient1.getTotalPop()
    average_viruses = [x / numTrials for x in num_viruses]
    plotHelper(average_viruses)

def plotHelper(means):
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for v in self.getViruses()[:]:
            try:
                new_virus = v.reproduce(pop_density, self.getPrescriptions())
...
patient = TreatedPatient([virus1, virus2], 10)
patient.addPrescription("drug1")
for i in range(5):
    patient.update()
test = patient.getViruses()
test
#
# PROBLEM 4
#
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        for j in range(150):
            patient2.update()
            num_viruses[j] += patient2.getTotalPop()
            num_resistant_viruses[j] += patient2.getResistPop(['guttagonol'])
        patient2.addPrescription('guttagonol')
        for k in range(150, 300):
            patient2.update()
            num_viruses[k] += patient2.getTotalPop()
            num_resistant_viruses[k] += patient2.getResistPop(['guttagonol'])
    average_viruses = [x / numTrials for x in num_viruses]
    average_resistant = [x / numTrials for x in num_resistant_viruses]
    plotHelper2(average_viruses, average_resistant)

def plotHelper2(means, means2):
    pylab.plot(means, label = 'Non Resistant')
    pylab.plot(means2, label = 'Resistant')
...
#simulationWithDrug(1, 20, 1.0, 0.0, {"guttagonol": True}, 1.0, 3)
